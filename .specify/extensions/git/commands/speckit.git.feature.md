...
Create and switch to a new git feature branch for the given specification. This
command handles **branch creation only** — the spec directory and files are
created by the core `/speckit.specify` workflow.
...
- `FEATURE_NUM` is extracted from the name if it starts with a numeric prefix,
  otherwise set to the full branch name
...
2. Check `.specify/init-options.json` for `branch_numbering` value (backward
   compatibility)
...
- Do NOT pass `--number` — the script determines the correct next number
  automatically
- Always include the JSON flag (`--json` for Bash, `-Json` for PowerShell) so
  the output can be parsed reliably
...
- The script still outputs `BRANCH_NAME` and `FEATURE_NUM` so the caller can
  reference them
...
- `BRANCH_NAME`: The branch name (e.g., `003-user-auth` or
