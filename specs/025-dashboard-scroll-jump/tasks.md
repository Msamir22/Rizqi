...
- [X] T014 [US1] **Fix applied at SafeArea provider boundary (not at TopNav layout).** Per FD-1=No, FD-2=No, FD-4=No: the root cause is the `react-native-safe-area-context` cold-start inset race (H1 confirmed; insets.top 0 → 28.44 between render 1 and render 2). Edited `apps/mobile/app/_layout.tsx` to import `initialWindowMetrics` and pass `initialMetrics={initialWindowMetrics}` to `<SafeAreaProvider>`. This provides synchronous, correct insets on the first render, eliminating the SafeAreaView padding jump that manifested as the TopNav "clip + snap". `TopNav` layout in `apps/mobile/app/(tabs)/index.tsx` was NOT modified — FD-1 ruled out the extract-from-ScrollView restructure
...
- [ ] T019 [US1] Manual verification pass on the primary device per `specs/025-dashboard-scroll-jump/quickstart.md` "Verification" section for the cold-start scenarios: SC-001 (TopNav fully visible first frame), SC-002 (zero auto-scroll during load), SC-003 (no snap animation). Capture the post-fix recording as `after-android-navbar.mp4`
